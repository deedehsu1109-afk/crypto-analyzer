...
_CURRENCIES = ["TWD", "USD", "EUR", "JPY", "CNY", "HKD", "OTHER"]
...
_COLS = [
    ...
    ("銀行",       "bank_name",           120),
    ("帳號",       "account_no",          160),
    ("對方戶名",   "counterpart_name",    120),
    ("對方帳號",   "counterpart_account", 160),
    ("金額",       "amount",               90),
    ("幣別",       "currency",             60),
    ("餘額",       "balance",              90),
    ("備註",       "notes",               160),
...
class BankTxDialog(ctk.CTkToplevel):
    ...
    def _entry(self, parent, row, placeholder="", width=260) -> ctk.CTkEntry:
    ...
    def _build(self):
        ...
        # 日期 + 時間
        self._lbl(f, "日期：", 0)
        date_f = ctk.CTkFrame(f, fg_color="transparent")
        date_f.grid(row=0, column=1, padx=(4, 12), pady=5, sticky="w")
        self._date_e = ctk.CTkEntry(date_f, font=("Consolas", 11),
                                    placeholder_text="YYYY-MM-DD", width=130)
        self._date_e.pack(side="left")
        ctk.CTkLabel(date_f, text="  時間：",
                     font=("Microsoft JhengHei", 11, "bold")).pack(side="left")
        self._time_e = ctk.CTkEntry(date_f, font=("Consolas", 11),
                                    placeholder_text="HH:MM", width=80)
        self._time_e.pack(side="left")

        # 方向
        self._lbl(f, "方向：", 1)
        self._dir_var = tk.StringVar(value=_DIRECTIONS[0])
        ctk.CTkOptionMenu(f, values=_DIRECTIONS, variable=self._dir_var,
                          font=("Microsoft JhengHei", 11), width=120).grid(
            row=1, column=1, padx=(4, 12), pady=5, sticky="w")

        # 銀行
        self._lbl(f, "銀行：", 2)
        self._bank_e = self._entry(f, 2, "如：玉山銀行（808）")

        # 帳號
        self._lbl(f, "帳號：", 3)
        self._acc_e = self._entry(f, 3, "帳戶號碼", width=300)

        # 對方戶名
        self._lbl(f, "對方戶名：", 4)
        self._cname_e = self._entry(f, 4)

        # 對方帳號
        self._lbl(f, "對方帳號：", 5)
        self._cacc_e = self._entry(f, 5, "", width=300)

        # 金額 + 幣別
        self._lbl(f, "金額：", 6)
        amt_f = ctk.CTkFrame(f, fg_color="transparent")
        amt_f.grid(row=6, column=1, padx=(4, 12), pady=5, sticky="w")
        self._amt_e = ctk.CTkEntry(amt_f, font=("Consolas", 11),
                                   placeholder_text="0.00", width=130)
        self._amt_e.pack(side="left")
        ctk.CTkLabel(amt_f, text="  幣別：",
                     font=("Microsoft JhengHei", 11, "bold")).pack(side="left")
        self._cur_var = tk.StringVar(value=_CURRENCIES[0])
        ctk.CTkOptionMenu(amt_f, values=_CURRENCIES, variable=self._cur_var,
                          font=("Microsoft JhengHei", 11), width=90).pack(side="left")

        # 餘額
        self._lbl(f, "餘額：", 7)
        self._bal_e = ctk.CTkEntry(f, font=("Consolas", 11),
                                   placeholder_text="（選填）", width=130)
        self._bal_e.grid(row=7, column=1, padx=(4, 12), pady=5, sticky="w")

        # 備註
        self._lbl(f, "備註：", 8)
        self._notes_e = self._entry(f, 8)

        # 來源文件
        self._lbl(f, "來源文件：", 9)
        self._src_e = self._entry(f, 9, "文件路徑或說明", width=320)

        # 按鈕
        btn_f = ctk.CTkFrame(f, fg_color="transparent")
        btn_f.grid(row=10, column=0, columnspan=2, pady=(10, 4))
    ...
    def _fill(self, row: dict):
        ...
        self._dir_var.set(row.get("direction", _DIRECTIONS[0]))
        _set(self._bank_e,  "bank_name")
        _set(self._acc_e,   "account_no")
        _set(self._cname_e, "counterpart_name")
        ...
        bal = row.get("balance")
        self._bal_e.delete(0, "end")
        if bal is not None:
            self._bal_e.insert(0, str(bal))
    ...
    def _save(self):
        ...
        try:
            ...
            return
        bal_str = self._bal_e.get().strip()
        try:
            balance = float(bal_str) if bal_str else None
        except ValueError:
            messagebox.showwarning("格式錯誤", "餘額請輸入數字", parent=self)
            return
        data = {
            "id":                  self.row.get("id"),
            ...
            "counterpart_name":    self._cname_e.get().strip(),
        ...
        _db.upsert_bank_transaction(self.case_id, data)
        if self.on_save:
            self.on_save()
        self.destroy()


class CaseBankTxPanel(ctk.CTkFrame):
    def __init__(self, parent, case_id: int):
        super().__init__(parent, fg_color=BG_PANEL, corner_radius=0)
        self.case_id = case_id
        self._active_dialog: BankTxDialog | None = None
        self.grid_columnconfigure(0, weight=1)
    ...
    def _build(self):
        ...
        style = ttk.Style()
        style.configure("BankTx.Treeview",
                        background=BG_ROW_ODD, foreground="white",
                        fieldbackground=BG_ROW_ODD, rowheight=22,
                        font=("Consolas", 10))
        style.configure("BankTx.Treeview.Heading",
                        background="#2a3556", foreground="#aac4ff",
                        font=("Microsoft JhengHei", 10, "bold"))
        style.map("BankTx.Treeview", background=[("selected", "#3a5a9a")])
        self._tree.configure(style="BankTx.Treeview")
    ...
    def _load(self):
        for r in self._tree.get_children():
            self._tree.delete(r)
        rows = _db.get_bank_transactions(self.case_id)
    ...
    def _add(self):
        if self._dialog_open():
            return
        self._active_dialog = BankTxDialog(self, self.case_id, on_save=self._load)
    ...
    def _edit(self):
        ...
        if not row:
            ...
            return
        self._active_dialog = BankTxDialog(self, self.case_id, row=row, on_save=self._load)
    ...
    def _delete(self):
        ...
        if not messagebox.askyesno("確認刪除",
            ...
            return
        for iid in sel:
            _db.delete_bank_transaction(int(iid))
        self._load()
